...
import NativeShare from "@/components/store/NativeShare";
...
export async function generateMetadata({ params }: PageProps) {
  try {
    ...
    const post = await getDocRest("journal_entries", id);
    if (!post || !post.is_public) return { title: "Post Not Found" };
...
export default async function BlogPostPage({ params }: PageProps) {
  const { id } = await params;
  const post = await getDocRest("journal_entries", id) as JournalEntry | null;
  if (!post || !post.is_public) notFound();

  // Fetch linked varieties
  let linkedVarieties: any[] = [];
  if (post.variety_ids && post.variety_ids.length > 0) {
    const results = await Promise.all(post.variety_ids.map(vid => getDocRest("varieties", vid)));
    linkedVarieties = results.filter(Boolean);
  }

  const title = post.public_title || post.title;
  const bodyText = post.public_body || post.body;
  const dateStr = typeof post.created_at === 'string' ? post.created_at : '';
      ...
      <div className="bg-root pt-32 pb-16 px-6 text-center text-linen relative overflow-hidden">
        ...
        <div className="relative z-20 max-w-3xl mx-auto">
          ...
          <p className="font-dm-sans font-bold text-linen/70 uppercase tracking-widest text-xs">
            {dateObj instanceof Date && !isNaN(dateObj.getTime()) ? format(dateObj, 'MMMM d, yyyy') : ''} · By {post.author}
          </p>
        </div>
      </div>
